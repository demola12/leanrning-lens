...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/student/library/list?profile_id=${studentId}`)
      .then((r) => r.json())
      ...
      <div className="flex items-center gap-1 mb-6 p-1 bg-gray-100 rounded-lg w-fit">
        ...
        >
          ...
          {!libraryLoading && libraryItems.length > 0 && (
            <span className="text-xs bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">{libraryItems.length}</span>
          )}
        </button>
      </div>
        ...
        <>
          ...
          {assignments.length === 0 ? (
            <div className="py-16 text-center rounded-lg border border-dashed border-gray-200">
              ...
              <button onClick={() => router.push("/teacher/assignments")} className="mt-3 text-sm font-semibold text-primary hover:text-primary-dark transition-colors">Go to Assignments</button>
            </div>
          ) : (
                  ...
                  <motion.div key={a.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-gray-900 truncate">{a.assignment?.title || "Untitled"}</h3>
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
                        ...
                        {a.due_date && <><span className="w-1 h-1 rounded-full bg-gray-300" /><span className={overdue ? "text-red-400" : ""}>Due {new Date(a.due_date).toLocaleDateString()}</span></>}
                        <span className="w-1 h-1 rounded-full bg-gray-300" />
                        <span>{new Date(a.created_at).toLocaleDateString()}</span>
                      </div>
                      {a.score !== null && graded.length > 0 && (
                        <div className="mt-1.5 flex items-center gap-3 text-xs">
                          <span className={`font-semibold ${a.score >= 70 ? "text-emerald-600" : a.score >= 40 ? "text-amber-600" : "text-red-500"}`}>{a.score}%</span>
                          <span className="text-gray-400">{correct}/{graded.length} correct</span>
                          {a.submission?.earned_points !== undefined && <span className="text-gray-400">{a.submission.earned_points}/{a.submission.total_points} pts</span>}
                        </div>
                      )}
                    </div>
                    <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${st.bg} ${st.color}`}>{st.label}</span>
                    {(a.status === "submitted" || a.status === "graded") && (
                      <button onClick={() => router.push(`/teacher/assignments/submission?id=${a.id}`)} className="p-2 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/5 transition-all">
        ...
        <>
          {libraryLoading ? (
            ...
            <div className="py-16 text-center rounded-lg border border-dashed border-gray-200">
              ...
              <p className="text-sm text-gray-500">No library items uploaded yet.</p>
              <p className="text-xs text-gray-400 mt-1">Students can upload images and PDFs from their Library page.</p>
            </div>
          ) : (
                ...
                <motion.div key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-9 h-9 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center shrink-0">
                      {item.file_type === "image" ? <Image className="w-4 h-4 text-blue-500" /> :
                       item.file_type === "pdf" ? <FileText className="w-4 h-4 text-red-500" /> :
                    ...
                    {item.viewed ? (
                      <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full"><CheckCheck className="w-3 h-3" /> Viewed</span>
                    ) : (
                      <span className="text-[10px] font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">New</span>
                    )}
                  </div>
                  ...
                  <a href={item.file_url} target="_blank" rel="noopener noreferrer"
                    onClick={async () => {
                      if (!item.viewed) {
                        await fetch("/api/student/library/viewed", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ item_id: item.id }) });
                        setLibraryItems((prev) => prev.map((i) => i.id === item.id ? { ...i, viewed: true, viewed_at: new Date().toISOString() } : i));
                      }
                    }}
                    className="flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary-dark transition-colors">
                    <ExternalLink className="w-3 h-3" />
                    Open {item.file_type === "pdf" ? "PDF" : "Image"}
                  </a>
                </motion.div>
              ))}
            </div>
