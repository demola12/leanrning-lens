...
export default function EditAssignmentPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const res = await fetch(`/api/assignments/get?id=${assignmentId}`);
      const data = await res.json();
      if (data.id) {
        setTitle(data.title);
        setDescription(data.description || "");
        const qs = (data.questions || []).map((q: any) => ({ ...q, id: q.id || generateId() }));
        setQuestions(qs.length > 0 ? qs : [createQuestion("multiple_choice")]);
        setActiveQuestion(qs[0]?.id || null);
        const tl = data.time_limit || 0;
        setTimeLimitEnabled(tl > 0);
        setTimeLimitMinutes(tl > 0 ? tl : 30);
      }
      setLoading(false);
    };
    load();
  ...
  const removeQuestion = (id: string) => {
    ...
    if (activeQuestion === id) {
      ...
      setActiveQuestion(questions[Math.max(0, idx - 1)]?.id || null);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-white shrink-0">
        <button onClick={() => router.back()} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-all">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <button onClick={() => setTimeLimitEnabled(!timeLimitEnabled)} className={`relative w-8 h-4 rounded-full transition-colors ${timeLimitEnabled ? "bg-primary" : "bg-gray-200"}`}>
            ...
            {timeLimitEnabled ? (
              <div className="flex items-center gap-1">
                <input type="number" min={1} max={999} value={timeLimitMinutes} onChange={(e) => setTimeLimitMinutes(Math.max(1, parseInt(e.target.value) || 1))} className="w-12 px-1.5 py-0.5 rounded-md border border-gray-200 text-xs font-semibold text-center focus:outline-none focus:ring-2 focus:ring-primary/20" />
          ...
          <button onClick={() => setPreview(!preview)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${preview ? "bg-primary text-white" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}>
            {preview ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            {preview ? "Edit" : "Preview"}
          </button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary-dark transition-all disabled:opacity-60">
            ...
            <div className="p-4 border-b border-gray-100">
              <div className="relative group">
                <button className="flex items-center justify-between w-full px-4 py-2.5 rounded-lg bg-white border border-gray-200 text-sm font-semibold text-gray-700 hover:border-gray-300 transition-all">
                  <span className="flex items-center gap-2"><Plus className="w-4 h-4 text-primary" />Add Question</span>
                ...
                <AnimatePresence initial={false}>
                  {questions.map((q, i) => (
                    <Reorder.Item
                      ...
                      value={q}
                      as="div"
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-lg cursor-pointer transition-colors ${activeQuestion === q.id ? "bg-white border border-gray-200 shadow-sm" : "hover:bg-white/80"}`}
                      onClick={() => setActiveQuestion(q.id)}
                    >
                      <GripVertical className="w-4 h-4 text-gray-300 shrink-0 cursor-grab" />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-gray-400 uppercase">{q.type.replace("_", " ")}</div>
                        <div className="text-sm font-medium text-gray-900 truncate">{q.title || `Question ${i + 1}`}</div>
                      </div>
                      <div className="flex items-center gap-0.5">
                        <button onClick={(e) => { e.stopPropagation(); duplicateQuestion(q.id); }} className="p-1 rounded text-gray-300 hover:text-gray-500 hover:bg-gray-100 transition-all">
                          <Copy className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); removeQuestion(q.id); }} className="p-1 rounded text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all" disabled={questions.length <= 1}>
          ...
          {preview ? (
            <div className="max-w-2xl mx-auto p-8">
                  ...
                  <div key={q.id} className="p-6 rounded-lg border border-gray-100 bg-white shadow-sm">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <h3 className="text-base font-semibold text-gray-900 mt-0.5">{i + 1}. {q.title || "Untitled question"}</h3>
                        {q.image_url && <img src={q.image_url} alt="" className="mt-3 max-h-40 rounded-lg border border-gray-100 object-contain" />}
                      </div>
                      ...
                      <div className="space-y-2">
                        {q.options?.map((opt, oi) => (
                          <label key={oi} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:border-gray-200 cursor-pointer transition-all">
                            ...
                            <span className="text-sm text-gray-700">{opt || `Option ${String.fromCharCode(65 + oi)}`}</span>
                          </label>
                        ))}
                      </div>
                    )}
                    ...
                    {q.type === "short_answer" && <input type="text" placeholder="Type your answer..." className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />}
                    {q.type === "essay" && <textarea rows={4} placeholder="Write your essay..." className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none" />}
                    {q.type === "rating" && (
                      <div className="flex items-center gap-2">
                        {[1, 2, 3, 4, 5].map((n) => (
              ...
              <div className="mt-8 text-center">
                <button className="px-8 py-3 rounded-lg bg-primary text-white font-semibold text-sm hover:bg-primary-dark transition-all shadow-sm shadow-primary/25">Submit Assignment</button>
              </div>
            </div>
          ) : (
            <div className="max-w-2xl mx-auto p-8">
                  ...
                  <div key={q.id} className="p-6 rounded-lg border border-gray-100 bg-white shadow-sm">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          <input type="text" defaultValue={q.title} onChange={(e) => updateQuestion(q.id, { title: e.target.value })}
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          <input type="number" min={1} defaultValue={q.points} onChange={(e) => updateQuestion(q.id, { points: Math.max(1, parseInt(e.target.value) || 1) })}
                      ...
                      {q.image_url && (
                        <div className="relative inline-block">
                          <img src={q.image_url} alt="" className="max-h-48 rounded-lg border border-gray-100 object-contain" />
                          <button onClick={() => updateQuestion(q.id, { image_url: undefined })}
                        ...
                        <div className="space-y-2">
                          {q.options?.map((opt, i) => (
                            <div key={i} className="flex items-center gap-2">
                              ...
                              <input type="text" defaultValue={opt} onChange={(e) => updateOption(q.id, i, e.target.value)}
                              ...
                              <button onClick={() => updateQuestion(q.id, { correctAnswer: String.fromCharCode(65 + i) })}
                                className={`text-xs font-semibold px-2 py-1 rounded-lg transition-colors ${q.correctAnswer === String.fromCharCode(65 + i) ? "bg-emerald-50 text-emerald-600" : "text-gray-400 hover:text-gray-600"}`}>Correct</button>
                            </div>
                          ))}
                        </div>
                      )}
                      {q.type === "true_false" && (
                        <div className="flex gap-3">
                          {["True", "False"].map((val) => (
                            <button key={val} onClick={() => updateQuestion(q.id, { correctAnswer: val })}
                              className={`flex-1 py-3 rounded-lg border-2 text-sm font-semibold transition-all ${q.correctAnswer === val ? "border-primary bg-primary/5 text-primary" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>{val}</button>
                          ))}
                        </div>
                      )}
                      {q.type === "short_answer" && (
                        <div>
                          <input type="text" defaultValue={q.correctAnswer || ""} onChange={(e) => updateQuestion(q.id, { correctAnswer: e.target.value })}
                            placeholder="Expected answer (optional)" className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
